...
class dan_ResNetf(dan_ResNet):
    ...
    def setup(self, layers, strides,layertype,hardness, compress_layer=True,inpch=1,oupch=512,expf=1.0):
        ...
        self.layer1 = self.LAYER( int(32*expf),int(32*expf), layers[0],stride=strides[1])
        self.layer2 = self.LAYER( int(32*expf),int(64*expf), layers[1], stride=strides[2])
        self.layer3 = self.LAYER( int(oupch//8*expf),int(oupch//4*expf), layers[2], stride=strides[3])
        self.layer4 = self.LAYER(int(oupch//4*expf),int(oupch//2*expf), layers[3], stride=strides[4])
        if(compress_layer):
            self.layer5 = self.LAYER( int(oupch//2*expf),int(oupch*expf), layers[4], stride=strides[5])
        else:
            self.layer5 = self.LAYER(int(oupch//2*expf), oupch, layers[4], stride=strides[5])
...
def res_naive_lens45F(strides, compress_layer,hardness,inpch=1,oupch=512,expf=1):
    model = dan_ResNetf( [3, 4, 6, 6, 3], strides,None,hardness, compress_layer,inpch=inpch,oupch=oupch,expf=expf)
    return model