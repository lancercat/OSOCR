...
'''
class neko_base_Feature_ExtractorF(nn.Module):
    def __init__(self, strides, compress_layer, input_shape,hardness=2,oupch=512,expf=1):
        super(neko_base_Feature_ExtractorF, self).__init__()
        self.model = rescco.res_base(strides, compress_layer,hardness,oupch=oupch,inpch=input_shape[0],expf=expf)
...
class neko_base_Feature_ExtractorF_thicc(nn.Module):
    def __init__(self, strides, compress_layer, input_shape,hardness=2,oupch=512,expf=1.5):
        super(neko_base_Feature_ExtractorF_thicc, self).__init__()
        self.model = rescco.res_base45_thicc(strides, compress_layer,hardness,oupch=oupch,inpch=input_shape[0])
        self.input_shape = input_shape
